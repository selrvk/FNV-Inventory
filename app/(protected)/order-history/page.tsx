...
export default async function OrderHistory() {
  ...
  // Fetch orders with status CONFIRMED or CANCELLED
  ...
  const { data: orderItems, error: itemsError } = await supabase
    .from("order_item")
    .select("*, items(name, unit)") // join item info
  ...
  return (
    <div className="container mx-auto py-6">
      <h1 className="text-blue-900 text-3xl font-bold mb-6">ORDER HISTORY</h1>
                  ...
                  <div className="mt-2 border-t pt-2">
                    ...
                    <ul className="space-y-1 text-sm text-gray-600">
                      {itemsForOrder.map(item => (
                        <li key={item.id}>
                          {item.items.name} — {item.quantity} {item.items.unit} @ ₱{item.price_at_time.toFixed(2)}
                        </li>
                      ))}
                    </ul>
                  </div>
